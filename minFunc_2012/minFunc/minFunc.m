...
for i = 1:maxIter
    ...
    if options.inter % note: put this before any break in if's
        % record intermediate images during iteration, Yi Guo,07.23.2015
        x_inter(:,i)=x;
%        imagesc(real(reshape(x,[256 150])),[0 0.3]);axis image; axis off; drawnow;
    end
    ...
    if max(abs(t*d)) < progTol % original <=
        exitflag=2;
